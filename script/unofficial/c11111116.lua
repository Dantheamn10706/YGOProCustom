...
function s.initial_effect(c)
    ...
    -- Add from Graveyard instead of drawing
...
-- Activation condition: same as Puzzle
function s.act_condition(e,tp,eg,ep,ev,re,r,rp)
    return Duel.GetLP(tp)<Duel.GetLP(1-tp)
        or Duel.IsExistingMatchingCard(function(c)
            return c:IsSetCard(0xf0f0) and c:IsFaceup()
        end,tp,0,LOCATION_ONFIELD+LOCATION_GRAVE,1,nil)
end

-- Draw Phase replacement condition
function s.draw_condition(e,tp,eg,ep,ev,re,r,rp)
    return Duel.GetTurnPlayer()==tp
end

-- Target: must have card in Graveyard to add
...
-- Operation: add 1 card from Graveyard to hand
