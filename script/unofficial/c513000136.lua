...
function s.adjustop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:GetAttack()~=c:GetBaseAttack()
		or c:GetDefense()~=c:GetBaseDefense()
		or c:IsHasEffect(EFFECT_CANNOT_ATTACK_ANNOUNCE) then
...
function s.resetOriginalStatsOperation(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if not c or not c.GetOriginalAttack then return end

	if c:GetAttack()~=c:GetOriginalAttack() then
		c:SetAttack(c:GetOriginalAttack())
	end
	if c.GetOriginalDefense and c:GetDefense()~=c:GetOriginalDefense() then
		c:SetDefense(c:GetOriginalDefense())
	end
end
...
function s.efilter(e,te)
	...
	if tp~=e:GetHandlerPlayer() then
		...
		if te:IsActiveType(TYPE_SPELL+TYPE_TRAP+TYPE_MONSTER)
			and (te:GetCategory() & (
				CATEGORY_DESTROY+
				CATEGORY_TOHAND+
				CATEGORY_TODECK+
...
function s.stgop(e,tp,eg,ep,ev,re,r,rp)
	...
	for _,eff in ipairs(effs) do
		if eff:GetOwner()~=c
			and not eff:GetOwner():IsCode(0)
			and not eff:IsHasProperty(EFFECT_FLAG_IGNORE_IMMUNE)
			and eff:GetCode()~=EFFECT_SPSUMMON_PROC
			and (eff:GetTarget()==aux.PersistentTargetFilter
				or not eff:IsHasType(EFFECT_TYPE_GRANT+EFFECT_TYPE_FIELD)) then
			eff:Reset()
		end
	end
end
...
function s.spchk(e,tp,eg,ep,ev,re,r,rp)
	...
	-- Capture the Special Summon origin immediately.
	-- Label 1 = from GY, label 2 = from banished.
	local return_label=0
	...
	-- Only Special Summons from GY or banished get the End Phase return.
	-- Special Summons from hand, Deck, Extra Deck, etc. stay on the field.
	if return_label~=0 then
		local e1=Effect.CreateEffect(c)
		e1:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
		e1:SetCode(EVENT_PHASE+PHASE_END)
		e1:SetRange(LOCATION_MZONE)
		e1:SetCountLimit(1)
		e1:SetLabel(return_label)
		e1:SetReset(RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END)
		e1:SetOperation(s.epreturn)
		c:RegisterEffect(e1)
	end
	...
	if te
		...
		and #tg==1
		and c:IsCanBeEffectTarget(te)
		and Duel.SelectEffectYesNo(tp,c,aux.Stringid(68823957,1)) then
		Duel.ChangeTargetCard(ev+1,Group.FromCards(c))
	end
end

function s.epreturn(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local return_label=e:GetLabel()
	e:Reset()

	-- If Slifer is no longer face-up in the Monster Zone, do nothing.
	if not c:IsLocation(LOCATION_MZONE) then return end

	if return_label==1 then
		Duel.SendtoGrave(c,REASON_EFFECT)
	elseif return_label==2 then
...
function s.atkcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return (not c:IsHasEffect(EFFECT_CANNOT_ATTACK_ANNOUNCE)
		and not c:IsHasEffect(EFFECT_FORBIDDEN)
...
function s.atktg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then
		return eg:IsContains(chkc) and s.atkfilter(chkc,1-tp,e)
	end
	if chk==0 then
		return eg:IsExists(s.atkfilter,1,nil,1-tp,e)
	end
