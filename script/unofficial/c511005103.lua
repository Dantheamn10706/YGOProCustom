...
if s then
    ...
    function s.op(e, tp, eg, ep, ev, re, r, rp)
        while true do
            local opts = {
                ...
                aux.Stringid(id, 7),  -- Reshuffle hand & redraw
                aux.Stringid(id, 8),  -- Return selected & redraw
    ...
    function s.do_reshuffle_redraw(p)
        -- Send the entire hand back into the deck, shuffle, redraw the same count.
        local hand_n = Duel.GetFieldGroupCount(p, LOCATION_HAND, 0)
        if hand_n == 0 then return end
        local hg = Duel.GetFieldGroup(p, LOCATION_HAND, 0)
        Duel.SendtoDeck(hg, p, 0, REASON_RULE)
        Duel.ShuffleDeck(p)
        Duel.Draw(p, hand_n, REASON_RULE)
    end
    function s.do_swap_random(p)
        -- User picks which hand cards to return to the deck; deck is shuffled;
        -- player draws the same count. Random side is the DECK (via shuffle+draw),
        -- not the hand.
        local hand_n = Duel.GetFieldGroupCount(p, LOCATION_HAND, 0)
        local deck_n = Duel.GetFieldGroupCount(p, LOCATION_DECK, 0)
        if hand_n == 0 or deck_n == 0 then return end
        local max_n = math.min(hand_n, deck_n)
        Duel.Hint(HINT_SELECTMSG, p, HINTMSG_TODECK)
        local hg = Duel.SelectMatchingCard(p, nil, p, LOCATION_HAND, 0, 1, max_n, nil)
        local n = #hg
        if n == 0 then return end
        Duel.SendtoDeck(hg, p, 0, REASON_RULE)
        Duel.ShuffleDeck(p)
        Duel.Draw(p, n, REASON_RULE)
    end
end
