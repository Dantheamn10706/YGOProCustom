...
function s.initial_effect(c)
	...
	--Sacred Beast archetype (from any zone)
	local ea=Effect.CreateEffect(c)
	ea:SetType(EFFECT_TYPE_SINGLE)
	ea:SetProperty(EFFECT_FLAG_SINGLE_RANGE+EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE)
	ea:SetRange(LOCATION_ALL)
	ea:SetCode(EFFECT_ADD_SETCODE)
	ea:SetValue(0x1145)
	c:RegisterEffect(ea)
	--Phantasm archetype (from any zone)
	local eb=Effect.CreateEffect(c)
	eb:SetType(EFFECT_TYPE_SINGLE)
	eb:SetProperty(EFFECT_FLAG_SINGLE_RANGE+EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE)
	eb:SetRange(LOCATION_ALL)
	eb:SetCode(EFFECT_ADD_SETCODE)
	eb:SetValue(0x145)
	c:RegisterEffect(eb)
