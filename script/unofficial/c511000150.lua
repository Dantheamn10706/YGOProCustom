...
function s.sealfilter(c,e)
	return c:IsFaceup() and c.IsCode(511002604) 
end
s.listed_names={511002604}
function s.val(c,e,tp,eg,ep,ev,re,r,rp)
	Debug.Message(Duel.IsExistingMatchingCard(aux.FilterFaceupFunction(Card.IsCode,511002604),tp,LOCATION_ONFIELD,0,1,nil))
	if Duel.IsExistingMatchingCard(aux.FilterFaceupFunction(Card.IsCode,511002604),tp,LOCATION_ONFIELD,0,1,nil) then
		return Duel.GetMatchingGroupCount(s.atkfilter,c:GetControler(),LOCATION_MZONE+LOCATION_GRAVE,0,nil)*600
	else
