...
function s.initial_effect(c)
	...
	e7:SetCategory(CATEGORY_EQUIP+CATEGORY_DAMAGE+CATEGORY_RECOVER)
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local tg=Duel.GetFieldGroup(p,0,LOCATION_MZONE):Select(p,1,1,nil):GetFirst()
	if tg and tg:IsCanBeEffectTarget(e) then
		Duel.Equip(tp,c,tg)
		--equip limit
		local e1=Effect.CreateEffect(c)
...
function s.atkcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return a and a==tc and Duel.IsExistingMatchingCard(Card.IsCode,tp,LOCATION_GRAVE,0,1,nil,31829185)
end
function s.atktg(e,tp,eg,ep,ev,re,r,rp,chk)
	local a=Duel.GetAttacker()
	local tc=e:GetHandler():GetEquipTarget()
	local atk=a:GetAttack()
	if chk==0 then return a and a==tc end
...
function s.atkop(e,tp,eg,ep,ev,re,r,rp)
	local a=Duel.GetAttacker()
	local tc=e:GetHandler():GetEquipTarget()
	if not a:IsCanBeEffectTarget(e) or not a or a~=tc  then return end
	local atk=a:GetAttack()
