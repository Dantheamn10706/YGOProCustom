...
function s.filter1(c,e,tp)
	...
	return (#pg<=0 or (#pg==1 and pg:IsContains(c))) and no and no>=101 and no<=107 
	and c:IsSetCard(0x48) and not c:IsSetCard(0x1048)
end
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	and chkc:IsLocation(LOCATION_MZONE|LOCATION_GRAVE|LOCATION_EXTRA) 
	and s.filter1(chkc,e,tp) end
	if chk == 0 then 
	return Duel.IsExistingMatchingCard(s.filter1,tp,LOCATION_MZONE|LOCATION_GRAVE|LOCATION_EXTRA,0,1,nil,e,tp) end
