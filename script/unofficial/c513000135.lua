...
function s.initial_effect(c)
	...
	-- Special Summon check (redirect/retarget helper)
	...
	-- Quick Effect: God Hand Crusher / Soul Energy Max
	...
	-- (GlobalCheck for Avatar remains outside this function, unchanged)
end
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local op=Duel.SelectEffect(tp,
		{true,aux.Stringid(id,1)}, -- God Hand Crusher
		{true,aux.Stringid(id,2)}) -- Soul Energy Max
	e:SetLabel(op)
	if op==1 then
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local op=e:GetLabel()
	if op==1 then
		-- God Hand Crusher: Destroy all opponent's monsters
		local g=Duel.GetMatchingGroup(nil,tp,0,LOCATION_MZONE,nil)
	...
	elseif op==2 and c:CanAttack() then
		...
		-- Must select an opponent's monster to attack (no direct attack)
