...
function c511002532.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local p=tp
	if Duel.IsExistingMatchingCard(c511002532.filter,1-tp,LOCATION_HAND+LOCATION_DECK+LOCATION_GRAVE,0,1,nil,e,1-tp) 
...
function c511002532.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c511002532.filter,tp,LOCATION_HAND+LOCATION_DECK+LOCATION_GRAVE,0,1,nil,e,tp) 
	...
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,p,LOCATION_HAND+LOCATION_DECK+LOCATION_GRAVE)
end
function c511002532.activate(e,tp,eg,ep,ev,re,r,rp)
	local p=Duel.GetChainInfo(0,CHAININFO_TARGET_PLAYER)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)>0 then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
		local g=Duel.SelectMatchingCard(tp,c511002532.filter,tp,LOCATION_HAND+LOCATION_DECK+LOCATION_GRAVE,0,1,1,nil,e,tp)
	...
	if p==PLAYER_ALL and Duel.GetLocationCount(1-tp,LOCATION_MZONE)>0 then
		...
		local g=Duel.SelectMatchingCard(1-tp,c511002532.filter,1-tp,LOCATION_HAND+LOCATION_DECK+LOCATION_GRAVE,0,1,1,nil,e,1-tp)
		if g:GetCount()>0 then
