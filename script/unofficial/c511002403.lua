...
function c511002403.dfilter(c)
	return c:IsType(TYPE_MONSTER) and c:IsDestructable()
end
function c511002403.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c511002403.dfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,e:GetHandler()) end
	local g=Duel.GetMatchingGroup(c511002403.dfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,nil)
	Duel.SetOperationInfo(0,CATEGORY_DICE,nil,0,tp,1)
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,g,g:GetCount(),0,0)
end
function c511002403.filter(c)
...
function c511002403.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if dc==a1 or dc==a2 then
		local g=Duel.GetMatchingGroup(c511002403.filter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,nil)
		local tc=g:GetFirst()
		while tc do
